...
--awful.util.tagnames = {  " ", " ", " ", " ", " ", " ", " ", " ", " ", " "  }
awful.util.tagnames = { " DEV ", " WWW ", " CHAT ", " MUS ", " SYS ", " DOC ", " VBOX ", " VID ", " GFX " }
...
local myawesomemenu = {
    ...
    -- { "arandr", "arandr" }, -- adjust resolution for multiples screens GUI utility based on xrander
...
globalkeys = my_table.join(
    ...
    {description = "show rofi drun mode", group = "hotkeys"}),
    ...
    awful.key({ modkey, altkey }, "w", function () awful.util.spawn( "whatsdesk" ) end,
        {description = "Whatsapp" , group = "gui apps" }),
    -- command line based mail reader
    -- awful.key({ modkey, altkey }, "e", function () awful.util.spawn( terminal.." -e neomutt" ) end,
    --     {description = "neomutt email" , group = "gui apps" }),
    
    -- File manager based on VIM
    ...
    -- Internal Relay Chat CLient 
    -- awful.key({ modkey, altkey  }, "i", function () awful.util.spawn( terminal.." -e irssi" ) end,
    --    {description = "irssi" , group = "terminal apps" }),
    
    awful.key({ modkey, altkey  }, "n", function () awful.util.spawn("simplenote" ) end,
        {description = "simplenote" , group = "gui apps" }),
    
    -- Web browser based on terminal
    ...
    -- This shit is a social media client o something like that for mastodon (a shady social media)
   ...
   -- awful.key({ modkey }, "Right", function () lain.util.tag_view_nonempty(1) end,
    ...
    awful.key({ altkey, "Shift"}, "Tab",
     ...
     {description = "switch focus", group = "layout"}),
   ...
   -- Layout manipulation
    ...
    awful.key({ altkey, }, "w", function () if beautiful.weather then beautiful.weather.show(5) end end,
              {description = "show weather", group = "widgets"}),

    -- Brightness
    awful.key({ }, "XF86MonBrightnessUp", 
    function ()
        if brightness < 1.0 then brightness = brightness + 0.05 end
        awful.util.spawn("xrandr --output eDP --brightness ".. brightness)
    end),
    awful.key({ }, "XF86MonBrightnessDown", 
    function () 
        if brightness > 0.1 then brightness = brightness - 0.05 end
            awful.util.spawn("xrandr --output eDP --brightness ".. brightness)
    end),
    ...
    function ()
        if brightness < 1.0 then brightness = brightness + 0.05 end
        awful.util.spawn(string.format("xrandr --output eDP --brightness %s", brightness)) 
    end),
    awful.key({ }, "XF86Mail", 
    function () 
        if brightness > 0.05 then brightness = brightness - 0.05 end
            awful.util.spawn(string.format("xrandr --output eDP --brightness %s", brightness))
    end),
